...
conn = MySQLdb.connect(host="localhost",user="root",password="root",db="login_info")
...
def upload_redirect():
    if(os.path.isfile(APP_ROOT[0]+"/image.jpeg")):
        os.remove(APP_ROOT[0] +"/image.jpeg")
    return render_template("upload.html")

@app.route("/upload", methods=['POST'])
def upload():
    print(APP_ROOT1[0])
    if not os.path.isfile(APP_ROOT1[0]+"/image.jpeg"):
        return render_template("upload.html",msg="spoof detected")
    id_folder = str(request.form['id_folder'])
    session['id_folder']= id_folder
    target = os.path.join(APP_ROOT1[0],"test/")
    ...
    if not os.path.isdir(target1):
        os.mkdir(target1)
    shutil.copyfile(APP_ROOT1[0]+"/"+"image.jpeg",target1+"image.jpeg")
    destination = APP_ROOT1[0] + "/" + "test/" + test_append + "/" + "image.jpeg"
...
def match():
    ...
    if os.path.isfile(destination):
        ...
        try:
            ...
            os.remove(APP_ROOT1[0]+"/image.jpeg")
...
def predict1(model):
    ...
    test_dir = APP_ROOT1[0]+"test/" + test_append
    ...
    for i in range(len(f_preds)):
        if(f_preds[i]==[]):
            ...
            os.remove(APP_ROOT1[0]+"/image.jpeg")
    excel = os.path.join(APP_ROOT1[0],"excel/")
...
def hello():
    ...
    im.save('image.jpeg')
    print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^>",APP_ROOT1)
    filepath = (APP_ROOT1[0] + "\image.jpeg")
    print("********************************************>>",filepath)
